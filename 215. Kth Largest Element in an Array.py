...
class Solution:
    ...
    def k_largest_rec(self, nums, l, r, k):
        p = self.partition(nums, l, r)
        if p == k:
            return nums[p]
        if p > k:
            return self.k_largest_rec(nums, l, p - 1, k)
        if p < k:
            return self.k_largest_rec(nums, p + 1, r, k)

    def findKthLargest(self, nums: List[int], k: int) -> int:
        return self.k_largest_rec(nums, 0, len(nums) - 1, len(nums)-k)
